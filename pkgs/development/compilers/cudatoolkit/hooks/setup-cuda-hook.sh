...
(( "$hostOffset" == -1 && "$targetOffset" == 0 )) || guard=Skipping
[[ -n ${cudaSetupHookOnce-} ]] && guard=Skipping

if (( "${NIX_DEBUG:-0}" >= 1 )) ; then
    echo "$guard hostOffset=$hostOffset targetOffset=$targetOffset setupCudaHook" >&2
else
    echo "$guard setup-cuda-hook" >&2
fi
...
setupCUDAToolkitCompilers() {
    ...
    if [[ -z "${nvccDontPrependCudartFlags-}" ]] ; then
        ...
        for pkg in "${!cudaOutputToPath[@]}" ; do
            ...
            if [[ -d "$path/include" ]] ; then
                export NVCC_APPEND_FLAGS+=" -I$path/include"
            fi
            if [[ -d "$path/lib" ]] ; then
                export NVCC_APPEND_FLAGS+=" -L$path/include"
            fi
        done
    fi
}
