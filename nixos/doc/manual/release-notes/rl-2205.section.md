...
- The interface of `cudnn` has changed:
  * The package now needs to be called with individual cuda-redist packages
    from `cudaPackages` namespace, like `libcublas`.
  * It does not anymore include `cudatoolkit` in `propagatedBuildInputs`

  This is part of migration to the redistributable `cudaPackages`.
  Downstream packages are now to consume the entire `cudaPackages`
  rather than `cudnn` directly.
