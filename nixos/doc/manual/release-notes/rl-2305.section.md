...
- As Singularity has renamed to [Apptainer](https://apptainer.org/news/community-announcement-20211130)
  to distinguish from [an un-renamed fork by Sylabs Inc.](https://apptainer.org/news/community-announcement-20211130),
  there are now two packages of Singularity/Apptainer:
  * `apptainer`: From `apptainer/apptainer`, which is the new repo after renaming. Its Go import path is `github.com/apptainer/apptainer`
  * `singularity`: From `sylabs/singularity`, which is the fork by Sylabs Inc.. Its Go import path is `github.com/sylabs/singularity`
