...
- `blender` now provides both the lastest stable and LTS versions under `blenderVersions`. `blender` and `blender-hip` now default to `blenderVersions.stable`, which points to the latest stable version. The latest LTS version is available with `blender-lts`. An attribute `isLTS` is added.

- `blender-with-packages` has moved to `blender.withPackages`, though `blender-with-packages` is still available for backwards compatibility for `blender` and `blender-hip`. `blender-with-packages` is now an alias for `blenderVersions.stable.withPackages`.
...
- All `blender` packages now passes its version of python under `blender.python`.
